...
Simulations used to investigate the performances of the different approaches to measure phylogenetic signal in species interactions in bipartite ecological networks are available in the folder "simulations" with an associated README. Functions to simulate the BipartiteEvol interaction networks are available in RPANDA, while the script used to simulate interaction networks with phylogenetic signal in the number of partners (and not their identity) can be found [here](https://github.com/BPerezLamarque/HOME/blob/master/Phylosignal_network/example/sim_signal_number_partners.R).
